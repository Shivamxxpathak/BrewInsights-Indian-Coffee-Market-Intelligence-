...
def main():
    ...
    demand=build_demand_series(df); forecasts,dm=forecast_demand(demand); demand.to_csv(out/"demand_by_period.csv",index=False); forecasts.to_csv(out/"demand_forecast.csv",index=False); dm.to_csv(out/"demand_model_comparison.csv",index=False)
    cities=build_city_features(df,pd.read_csv(external)); scored=score_market_entry(cities); scored.to_csv(out/"market_entry_validation.csv",index=False); clustered,cm=cluster_cities(cities); clustered.to_csv(out/"city_cluster_validation.csv",index=False); cm.to_csv(out/"city_clustering_metrics.csv",index=False)
